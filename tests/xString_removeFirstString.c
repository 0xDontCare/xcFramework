// unit test for xString_removeFirstString function in xString module
...
int main(void) {
    ...
    for (int i = 0; i < num_test_cases; i++) {
        ...
        xString_removeFirstString(str, sub);

        assert(str->len == test_cases[i].expected_len && "xString_removeFirstString() failed: length mismatch");
        for (int j = 0; j < test_cases[i].expected_len; j++) {
            assert(str->data[j] == (unsigned char)test_cases[i].expected[j] && "xString_removeFirstString() failed: content mismatch");
        }

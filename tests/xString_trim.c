// unit test for xString_trim function in xString module
...
int main(void) {
    struct test_case test_cases[] = {
        {"   Hello, World!   ", 19, "Hello, World!", 13},
        {"  \t\t  This is a test  \t  ", 25, "This is a test", 14},
    ...
    for (int i = 0; i < num_test_cases; i++) {
        ...
        assert(str->len == test_cases[i].expected_len && "xString_trim() failed: length mismatch");
        for (int j = 0; j < test_cases[i].expected_len; j++) {
            assert(str->data[j] == (unsigned char)test_cases[i].expected[j] && "xString_trim() failed: content mismatch");
        }

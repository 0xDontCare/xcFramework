// unit test for xString_toUppercase function in xString module
...
int main(void) {
    ...
    for (int i = 0; i < num_test_cases; i++) {
        ...
        xString_toUppercase(str);

        assert(str->len == test_cases[i].expected_len && "xString_toUppercase() failed: length mismatch");
        for (int j = 0; j < test_cases[i].expected_len; j++) {
            assert(str->data[j] == (unsigned char)test_cases[i].expected[j] && "xString_toUppercase() failed: content mismatch");
        }

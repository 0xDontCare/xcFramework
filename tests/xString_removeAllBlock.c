// unit test for xString_removeAllBlock function in xString module
...
int main(void) {
    ...
    for (int i = 0; i < num_test_cases; i++) {
        ...
        xString_removeAllBlock(str, (unsigned char *)test_cases[i].sub, test_cases[i].sub_len);

        assert(str->len == test_cases[i].expected_len && "xString_removeAllBlock() failed: length mismatch");
        for (int j = 0; j < test_cases[i].expected_len; j++) {
            assert(str->data[j] == (unsigned char)test_cases[i].expected[j] && "xString_removeAllBlock() failed: content mismatch");
        }
